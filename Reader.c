...
ReaderPointer readerCreate(i32 size, i32 increment, i32 mode) {
   ...
   if (size < 0 || increment < 0){
	...
	if (increment == 0){
		incremnent = READER_DEFAULT_INCREMENT;
	}
	...
	if (readerPointer->offset.wrte * (i32)sizeof(char) < readerPointer->size) {
		...
		case MODE_ADDIT:
			...
			if (newSize < 0 || newSize >= READER_MAX_SIZE) {
				readerPointer->flags |= FULL_FLAG; // Set the FULL flag if the new size is invalid
		...
		case MODE_MULTI:
			...
			if (newSize < 0 || newSize >= READER_MAX_SIZE){
				readerPointer ->flags |= FULL_FLAG; //set flag id new size is invlaid 
		...
		/* TO_DO: Defensive programming */
		if (tempReader == NULL) {
		readerPointer->flags |= FULL_FLAG; //SET THE FULL FLAG IF THE REALLOC FIALS 
		return NULL; 
	}
...
bool readerClear(ReaderPointer const readerPointer) {
	...
	/*reset offsets*/
	readerPointer -> offset.rd = 0;
	readerPointer -> offset.wr = 0;
...
*************************************************************
*/
bool readerSetMark(ReaderPointer const readerPointer, mark: i32) {
	/* TO_DO: Defensive programming */
	...
	/* TO_DO: Adjust mark */
	readerPointer-> offset.posMark = mark;
...
*************************************************************
*/
 readerPrint(ReaderPointer const readerPointer) {
	i32 cont = 0;
	char c;
...
bool readerRetract(ReaderPointer const readerPointer) {
	...
	if (readerPointer -> offset.rd > 0){
		readerPointer-> offset.rd--;
	}
	return true;
}
...
bool readerRestore(ReaderPointer const readerPointer) {
	...
	readerPointer-> offset.rd = readerPointer->offset.mark;
	return true;
}
